...
server <- function(input, output, session) {
  ...
  output$top_scorer_missing <- renderText({
    missing_scorer_pct <- filtered_data$df() |>
      summarise(
  ...
  output$total_countries <- renderText({
    ...
    total_countries <- filtered_data$df() |>
  ...
  output$overview <- renderEcharts4r({
    ...
    filtered_data$df() |>
      group_by(date = lubridate::floor_date(as.Date(date), "year")) |>
